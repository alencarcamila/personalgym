...
const WORKOUTS = [
  {
    ...
    exercises: [
      ...
      { id:"e2",  name:"Remada Curvada com Barra",     sets:4, repsMin:8,  repsMax:10, muscle:"Costas", secondary:["Bíceps","Core"],         tips:"Tronco a 45°, cotovelos próximos", errors:"Não arquear excessivamente a lombar", description:"Remada inclinada com barra livre." },
      ...
      { id:"e4",  name:"Remada Baixa Neutra",          sets:3, repsMin:10, repsMax:12, muscle:"Costas", secondary:["Bíceps","Core"],         tips:"Cotovelos a 45° do tronco", errors:"Não arredondar as costas", description:"Remada na polia baixa com pegada neutra." },
  ...
  },
  {
    ...
    cardio: "Escolha sua modalidade",
    exercises: [
      { id:"e21", name:"Spinning",                    sets:1, repsMin:40, repsMax:40, muscle:"Cardio", secondary:["Pernas"],           tips:"Cadência constante 80-100 RPM", errors:"Não pedalar com joelhos para dentro", description:"Ciclismo indoor de alta intensidade — duração em minutos." },
      { id:"e22", name:"Corrida",                     sets:1, repsMin:3,  repsMax:5,  muscle:"Cardio", secondary:["Pernas","Core"],    tips:"Passada natural, respire pelo nariz", errors:"Não inclinar excessivamente para frente", description:"Corrida em esteira ou ao ar livre — distância em km." },
      { id:"e23", name:"Escada + Caminhada Inclinada",sets:1, repsMin:20, repsMax:30, muscle:"Cardio", secondary:["Pernas","Glúteos"], tips:"Postura ereta, use o calcanhar", errors:"Não se apoiar no corrimão", description:"Subida de escadas + caminhada inclinada — duração em minutos." },
    ],
  },
];
...
function ActiveWorkout({ workout, history, customImages, onFinish, onBack, onUploadImage, savedState, onSaveState }) {
  // Cronômetro: usa tempo salvo se existir (persistência ao sair do app)
  ...
  // Persistir estado do treino em andamento no localStorage
  useEffect(()=>{
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{background:"#0f0f1a"}}>
      ...
      {/* SELETOR DE EXERCÍCIOS — ordem livre */}
      ...
      {curEx&&curState&&(
        <div className="px-4 flex-1 pb-6">
          <div className="rounded-2xl p-5 mb-3" style={{background:"#1a1a2e",border:"1px solid #6366f1"}}>
            <div className="flex items-start justify-between mb-3">
              <div>
            ...
            {/* SÉRIES — cada uma com carga e reps independentes */}
            <div className="space-y-3">
              {curState.sets.map((set,idx)=>(
                <div key={idx} className="rounded-xl p-3 transition-all"
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    {!set.done&&(idx===0||(idx>0&&curState.sets[idx-1]?.done))&&(
                      <span className="text-xs px-2 py-0.5 rounded-full font-bold"
                        style={{background:"#6366f1",color:"white"}}>Atual</span>
                    )}
                  </div>
                  <div className="grid grid-cols-2 gap-2 mb-2">
                    <div>
                      <label className="text-xs mb-1 block" style={{color:"#64748b"}}>Carga (kg)</label>
                      <input type="number" value={set.load} disabled={set.done}
                        ...
                        style={{background:"#1a1a2e",border:"1px solid #2d2d4e",opacity:set.done?0.6:1}}/>
                    </div>
                    <div>
                      <label className="text-xs mb-1 block" style={{color:"#64748b"}}>Reps</label>
                      <input type="number" value={set.reps} disabled={set.done}
...
export default function FitnessApp() {
  ...
  // Salva estado do treino em andamento para persistir se sair do app
  ...
  const handleWorkoutFinish = (completedExercises, duration) => {
    ...
    // Salva CADA SÉRIE individualmente no histórico
    for (const {exercise,entry} of completedExercises) {
      if(!newHistory[exercise.id]) newHistory[exercise.id]=[];
      newHistory[exercise.id].push({
        date:dateStr,
        load:entry.load,
        reps:entry.reps,
        volume:entry.reps*entry.load,
      });
    }
    ...
    saveLog(newLog);
    // Limpa estado salvo
    sv("gym_active_workout",null);
    sv("gym_active_state",null);
    setActiveSavedState(null);
    setActiveWorkout(null);
    setTab("home");
  };

  const handleStartWorkout = (w) => {
    // Se havia treino salvo de outro workout, limpa
    const saved = ld("gym_active_workout",null);
    if(saved&&saved.workoutId!==w.id){
      sv("gym_active_state",null);
      setActiveSavedState(null);
    }
    setActiveWorkout(w);
      ...
      {dayDetail&&(
        <Modal onClose={()=>setDayDetail(null)}>
          <div className="p-5">
            ...
            {dayDetail.data?.duration!=null&&(
              <div className="mt-3 text-sm" style={{color:"#64748b"}}>⏱ {dayDetail.data.duration} min</div>
            )}
              ...
              {WORKOUTS.map(w=>(
                <button key={w.id} onClick={()=>handleStartWorkout(w)}
                  ...
                  <div className="text-left">
                    ...
                    <div className="text-xs mt-0.5" style={{color:"#64748b"}}>{w.exercises.length} exercícios · {w.cardio}</div>
                  </div>
          ...
          {WORKOUTS.map(w=>(
            <div key={w.id} className="rounded-2xl mb-5" style={{background:"#1a1a2e",border:"1px solid #2d2d4e"}}>
              ...
              {w.exercises.map(ex=>{
                ...
                // Último treino: última entrada única por data
      ...
      {tab==="progress"&&(
        <div className="px-4 pt-6">
          ...
          {WORKOUTS.flatMap(w=>w.exercises)
            ...
            .slice(0,6)
            .map(ex=>{
              // Agrupa por data pegando a carga máxima do dia
              const byDate={};
              ...
              return (
                <div key={muscle} className="mb-3">
                  ...
                  <div className="h-2 rounded-full" style={{background:"#2d2d4e"}}>
                    <div className="h-2 rounded-full" style={{width:`${(vol/maxVol)*100}%`,background:"linear-gradient(90deg,#6366f1,#8b5cf6)"}}/>
                  </div>
                </div>
              );
            })}
                  ...
                  {isEditing?(
                    <div className="flex items-center gap-2">
                      <input value={editWeightVal} onChange={e=>setEditWeightVal(e.target.value)}
                        type="number" className="w-20 px-2 py-1 rounded-lg text-white text-center font-bold"
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button onClick={()=>{setEditWeightIdx(realIdx);setEditWeightVal(w.value.toString());}}
                        className="text-xs px-2 py-1 rounded-lg" style={{background:"#2d2d4e",color:"#94a3b8"}}>
                        ✏️
                      </button>
                    </div>
                  )}
